...
import { GameState, GameEvent, UnitState, PlayerId, UnitClass, Coord } from "./model";
...
function findUnit(state: GameState, owner: PlayerId, cls: UnitClass): UnitState {
  const unit = Object.values(state.units).find((u) => u.owner === owner && u.class === cls);
  if (!unit) {
...
function stepCreateGameAndArmies(rng: DefaultRNG): GameState {
  ...
  console.log("Initial:", "phase=", state.phase, "currentPlayer=", state.currentPlayer);
...
function stepInitiativeAndArena(state: GameState, rng: DefaultRNG): GameState {
...
function stepPlacementAutoBattle(state: GameState, rng: DefaultRNG): GameState {
  ...
  function nextUnplaced(state: GameState, owner: PlayerId): UnitState | null {
    return (
      Object.values(state.units).find((u) => u.owner === owner && !u.position && u.isAlive) ?? null
    );
  }
  ...
  while (state.phase === "placement") {
    ...
    console.log(`Placing ${unit.id} [${unit.class}] for ${current} at ${coordToString(pos)}`);
...
function stepKnightSearchAssassin(state: GameState, rng: DefaultRNG): GameState {
  ...
  let res = applyAction(state, { type: "unitStartTurn", unitId: knight.id }, rng);
  state = res.state;
  dumpEvents(res.events, "unitStartTurn(knight)");

  // 2) Рыцарь перемещается на соседнюю клетку (например, на (4,5))
  const moveTo = C(4, 5);
  res = applyAction(state, { type: "move", unitId: knight.id, to: moveTo }, rng);
  state = res.state;
...
function stepRiderPathAttack(state: GameState, rng: DefaultRNG): GameState {
  ...
  let res = applyAction(state, { type: "unitStartTurn", unitId: rider.id }, rng);
  state = res.state;
  dumpEvents(res.events, "unitStartTurn(rider)");

  // 2) Длинный ход по горизонтали, через врагов
  const target = C(5, 2);
  res = applyAction(state, { type: "move", unitId: rider.id, to: target }, rng);
  state = res.state;
...
function stepBerserkerAutoDefense(state: GameState, rng: DefaultRNG): GameState {
  ...
  let res = applyAction(state, { type: "unitStartTurn", unitId: attacker.id }, rng);
  state = res.state;
