...
function makeSharedAttackerWinRng(targets: number): SequenceRNG {
...
function testPapyrusLongLiverAdds2Hp() {
  let state = createEmptyGame();
...
function testPapyrusBlueBoneApplyPunishRefreshAndExpiry() {
  const rng = makeRngSequence([
    0.99, 0.99, 0.01, 0.01,
  ]);
  let state = createEmptyGame();
...
function testPapyrusSpaghettiGatingHealingClampAndChargeSpend() {
  const rng = makeRngSequence([]);
  let state = createEmptyGame();
  const a1 = createDefaultArmy("P1", { spearman: HERO_PAPYRUS_ID });
  const a2 = createDefaultArmy("P2");
...
function testPapyrusCoolGuyGatingLineHitsAndBlueBone() {
  const rng = makeRngSequence([
    0.99, 0.99,
    0.01, 0.01,
    0.01, 0.01,
...
function testPapyrusUnbelieverTriggersOnAlliedHeroDeathAndPersists() {
  const rng = makeRngSequence([0.99, 0.99, 0.01, 0.01]);
  let state = createEmptyGame();
  const a1 = createDefaultArmy("P1", {
    spearman: HERO_PAPYRUS_ID,
...
function testPapyrusOrangeBoneToggleAndEndTurnPunish() {
  const rng = makeRngSequence([0.99, 0.99, 0.01, 0.01]);
  let state = createEmptyGame();
  const a1 = createDefaultArmy("P1", { spearman: HERO_PAPYRUS_ID });
  const a2 = createDefaultArmy("P2");
...
function testPapyrusLongBoneAttackAndCoolGuyCostReduction() {
  const rng = makeRngSequence([
    0.99, 0.99,
    0.01, 0.01,
    0.01, 0.01,
...
function testPapyrusOssifiedBerserkerFeatureAfterTransformationOnly() {
  const rng = makeRngSequence([
    0.99, 0.99,
    0.01, 0.01,
    0.99, 0.99,
...
function testSansLongLiverAndSpearmanFeature() {
...
function testSansGasterBlasterGatingLineAndSpend() {
...
function testSansBadassJokeDebuffAndMovementLock() {
...
function testSansUnbelieverBoneFieldAndSleep() {
...
function testSansLastAttackCurse() {
  let { state, sans, enemy, enemy2 } = setupSansState();
...
function testUndyneToughSpearmanFeatureAndReach() {
...
function testUndyneThrowSpearFixedDamageAndSpearRain() {
  let { state, undyne, enemy } = setupUndyneState();
  state = setUnit(state, undyne.id, { position: { col: 4, row: 4 } });
...
function testUndyneEnergySpearGatingLineAndFreeImmortalCost() {
...
function testUndyneDirectionShiftDefenseRedirect() {
  let { state, undyne, ally, enemy, enemy2 } = setupUndyneState();
  state = setUnit(state, undyne.id, { position: { col: 4, row: 4 } });
  state = setUnit(state, enemy.id, { position: { col: 4, row: 5 } });
  state = toBattleState(state, "P2", enemy.id);
...
function testMettatonLongLiverCannotHideAndRiderMovement() {
...
function testMettatonRatingPassiveAndSpend() {
...
function testMettatonPoppinsGatingAreaAndRating() {
  let { state, mettaton, enemy } = setupMettatonState();
  const ally = Object.values(state.units).find(
...
function testMettatonExStageAndLaser() {
  let { state, mettaton, enemy } = setupMettatonState();
  const ally = Object.values(state.units).find(
    (u) => u.owner === "P1" && u.id !== mettaton.id && u.class === "knight"
  )!;
...
function testMettatonNeoGraceAndRiderPathUnlocks() {
  let { state, mettaton, enemy } = setupMettatonState();
  state = setUnit(state, mettaton.id, { position: { col: 4, row: 4 } });
  state = setUnit(state, enemy.id, { position: { col: 4, row: 6 } });
  state = toBattleState(state, "P1", mettaton.id);
...
function testMettatonBerserkerFeatureOnlyAfterNeo() {
  let { state, mettaton, enemy } = setupMettatonState();
  state = setUnit(state, mettaton.id, { position: { col: 4, row: 4 } });
  state = setUnit(state, enemy.id, { position: { col: 4, row: 5 } });
...
function testMettatonFinalChordGatingTargetsDamageAndSpend() {
  let { state, mettaton, enemy } = setupMettatonState();
  const enemy2 = Object.values(state.units).find(
    (u) => u.owner === "P2" && u.id !== enemy.id && u.class === "knight"
  )!;
