...
export function testRiderPathIgnoresHiddenEnemies() {
  ...
  state.units[rider.id] = {
    ...rider,
    position: { col: 0, row: 2 },
    hasMovedThisTurn: false,
    hasActedThisTurn: false,
  };
  state.units[enemy1.id] = {
    ...enemy1,
    position: { col: 2, row: 2 },
    isStealthed: true,
    stealthTurnsLeft: 3,
  };
  state.units[enemy2.id] = {
    ...enemy2,
...
export function testAssassinAttackFromStealth() {
  ...
  assert(next.units[attacker.id].isStealthed === false, "assassin should be revealed after attack");
  assert(
...
export function testHiddenHassanMissesDuolingoAndReveals() {
  ...
  assert(state.units[hassan.id].isStealthed, "a rejected attack must not consume Hassan's stealth");
  ...
  const attack = events.find((event) => event.type === "attackResolved");
...
export function testHiddenHassanHitsDuolingoAndReveals() {
  ...
  assert(
    events.filter((event) => event.type === "stealthRevealed" && event.unitId === hassan.id)
      .length === 1,
...
export function testHiddenAttackAutoDefendedStillReveals() {
  ...
  assert(
    defended.events.some((event) => event.type === "attackResolved" && !event.hit),
...
export function testAssassinAttackWithoutStealth() {
  ...
  assert(next.units[attacker.id].isStealthed === false, "assassin should remain not stealthed");
...
export function testSearchRevealsOnlyInRadius() {
  ...
  assert(next.units[nearEnemy.id].isStealthed === false, "near enemy should be revealed");
  assert(next.units[farEnemy.id].isStealthed === true, "far enemy should remain hidden");
  assert(
...
export function testSearchMoveBlockedAfterMove() {
  ...
  const mover = Object.values(state.units).find((u) => u.owner === "P1" && u.class === "spearman")!;
  const hidden = Object.values(state.units).find(
    (u) => u.owner === "P2" && u.class === "assassin",
  )!;
...
export function testAdjacentHiddenEnemyStaysHiddenAfterMove() {
  ...
  assert(
    next.units[mover.id].position!.col === 3 && next.units[mover.id].position!.row === 4,
    "mover should complete normal movement",
  );
  assert(next.units[hidden.id].isStealthed === true, "adjacent hidden unit should stay hidden");
...
export function testCannotAttackAfterSearchAction() {
  ...
  assert(attackAfter.events.length === 0, "attack should be blocked after search(action)");
...
export function testCannotSearchMoveAfterMove() {
  ...
  assert(searchAfterMove.events.length === 0, "search(move) should be blocked after move");
  assert.deepStrictEqual(
...
export function testRiderCannotEnterStealth() {
  ...
  assert(res.state.units[rider.id].isStealthed === false, "rider should remain visible");
...
export function testAssassinCanEnterStealth() {
  ...
  assert(res.state.units[assassin.id].isStealthed === true, "assassin should become stealthed");
...
export function testStealthOnlyForUnitsWithAbility() {
  ...
  for (const cls of nonStealthClasses) {
    ...
    const unit = Object.values(state.units).find((u) => u.owner === "P1" && u.class === cls)!;
...
export function testAllyCannotStepOnStealthedAlly() {
  ...
  assert(
    res.state.units[mover.id].position!.row === 3 && res.state.units[mover.id].position!.col === 3,
...
export function testUnknownStealthedEnemyDoesNotBlockArcherLine() {
  ...
  assert(
    resolved.state.units[hidden.id].isStealthed === true,
    "hidden line occupant should stay hidden",
  );
  assert(!revealEvent, "line scan should not reveal unknown hidden enemy");
  assert(
    resolved.state.knowledge["P1"][hidden.id] !== true,
    "line scan should not learn hidden enemy",
  );
...
export function testPathPassingAdjacentAfterStealthEntryDoesNotReveal() {
  ...
  const rider = Object.values(state.units).find((u) => u.owner === "P2" && u.class === "rider")!;
...
export function testEnemyCanStepOnRealStealthEntryCellWithoutReveal() {
  ...
  const mover = Object.values(state.units).find((u) => u.owner === "P2" && u.class === "knight")!;
...
export function testRevealDisplacesCoLocatedHiddenUnitDeterministically() {
  ...
  assert(
    aoe.events.some((event) => event.type === "stealthRevealed" && event.unitId === hidden.id),
...
export function testRealStealthEntryDoesNotBlockArcherLine() {
  ...
  const visible = Object.values(state.units).find((u) => u.owner === "P1" && u.class === "knight")!;
  const archer = Object.values(state.units).find((u) => u.owner === "P2" && u.class === "archer")!;
...
export function testNonRevealingAoEProjectionRedactsHiddenTargets() {
  ...
  assert(
    aoe.nextState.units[hidden.id].isStealthed === true,
    "hidden target should not be revealed",
  );
...
export function testNoStealthStackingOnEnter() {
  ...
  const stealthed = Object.values(state.units).find(
    (u) => u.owner === "P1" && u.class === "assassin",
  )!;
  const entering = Object.values(state.units).find(
  ...
  assert(
    enterEvent && enterEvent.type === "stealthEntered" && enterEvent.success === false,
    "enterStealth should fail",
  );
  assert(res.state.units[entering.id].isStealthed === false, "unit should remain visible");
  assert(
    res.state.units[entering.id].stealthAttemptedThisTurn === true,
    "stealth attempt should be spent",
  );
...
export function testStealthLasts3OwnTurnsThenExpiresOn4thStart() {
  ...
  assert(
    ...
    "stealth should remain on 1st own turn",
  );
  ...
  assert(
    res.state.units[assassin.id].isStealthed === true,
    "stealth should remain on 2nd own turn",
  );
  ...
  assert(
    res.state.units[assassin.id].isStealthed === true,
    "stealth should remain on 3rd own turn",
  );
...
export function testStealthRollLogged() {
  ...
  assert(typeof enterEvent.roll === "number", "stealthEntered should include roll");
  ...
  assert(res.state.units[assassin.id].isStealthed === true, "assassin should enter stealth");
...
export function testTricksterAoERevealsHiddenInArea() {
  ...
  const nearHidden = Object.values(state.units).find(
    (u) => u.owner === "P2" && u.class === "assassin",
  )!;
  const midHidden = Object.values(state.units).find(
    (u) => u.owner === "P2" && u.class === "archer",
  )!;
  const farHidden = Object.values(state.units).find(
    (u) => u.owner === "P2" && u.class === "spearman",
  )!;
  ...
  assert(
    resolved.state.units[nearHidden.id].isStealthed === false,
    "near hidden should be revealed",
  );
  assert(resolved.state.units[midHidden.id].isStealthed === false, "mid hidden should be revealed");
  assert(
    resolved.state.units[farHidden.id].isStealthed === true,
    "far hidden should remain stealthed",
  );
  assert(
    resolved.state.knowledge["P1"][nearHidden.id] === true,
    "caster knowledge should include revealed unit",
  );
...
export function testTricksterAoERevealsStealthedUnits() {
  ...
  const revealEvents = events.filter((e) => e.type === "stealthRevealed" && e.reason === "aoeHit");
  assert(
    revealEvents.some((e) => e.type === "stealthRevealed" && e.unitId === ally.id),
  ...
  assert(resolved.state.units[ally.id].isStealthed === false, "ally stealth should be cleared");
  assert(resolved.state.units[enemy.id].isStealthed === false, "enemy stealth should be cleared");
...
export function testCannotStealthTwicePerTurn() {
  ...
  assert(second.events.length === 0, "second stealth attempt should emit no events");
  assert.deepStrictEqual(
    second.state,
    resolvedFirst.state,
