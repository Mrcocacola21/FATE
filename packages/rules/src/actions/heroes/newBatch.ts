...
function applyDuolingoPush(state: GameState, unit: UnitState, action: AbilityAction): ApplyResult {
  ...
  if (
    !getDuolingoPushDestinations(state, unit, target).some((cell) =>
      coordsEqual(cell, destination!),
    )
  )
    return { state, events: [] };
  ...
  return {
    state: { ...committed.state, units: { ...committed.state.units, [moved.id]: moved } },
    events,
  };
}

function applyLucheLightRay(
  state: GameState,
  unit: UnitState,
  ...
  if (unit.heroId !== HERO_LUCHE_ID || !unit.position || (mode !== "line" && mode !== "aroundSelf"))
    return { state, events: [] };
  ...
  const target = mode === "line" ? parseCoord(data.target ?? data.center ?? data.line) : null;
...
function applyLucheFallingSun(state: GameState, unit: UnitState): ApplyResult {
  ...
  const targets = Object.values(state.units)
    .filter((target) => target.owner !== unit.owner && canAttackTarget(state, unit, target))
    .map((target) => target.id);
  const committed = commitNamedResource(
    state,
    unit,
  ...
  const queued = queueNewHeroAttacks(
    ...
    ids.ABILITY_LUCHE_BURNING_SUN,
    targets,
    { damageOverride: 2, ignoreBonuses: true, blindOnHit: true, center: unit.position, radius: 2 },
  );
  return { state: queued.state, events: [...committed.events, ...queued.events] };
}

function applyKanekiRegeneration(
  state: GameState,
  unit: UnitState,
  action: AbilityAction,
): ApplyResult {
  ...
  const maxHp =
    getHeroDefinition(unit.heroId)?.baseHpOverride ?? getUnitDefinition(unit.class).maxHp;
  ...
  return {
    state: { ...committed.state, units: { ...committed.state.units, [healed.id]: healed } },
    events: [
      ...committed.events,
      {
        type: "unitHealed",
        ...
        amount: heal,
        hpAfter: healed.hp,
        sourceAbilityId: ids.ABILITY_KANEKI_REGENERATION,
      },
    ],
  };
}

function applyZoroOniGiri(
  state: GameState,
  unit: UnitState,
  action: AbilityAction,
  isImpulse: boolean,
  ...
  const useDetermination =
    !isImpulse && hasHeroResourceSource(data, ids.ABILITY_ZORO_DETERMINATION, 2);
  if (!useCounter && !useDetermination) return { state, events: [] };
  if (!destination) return { state, events: [] };
  if (
    !getZoroOniGiriDestinations(state, unit, target).some((cell) => coordsEqual(cell, destination!))
  )
    return { state, events: [] };
  const committed = commitNamedResource(
    state,
  ...
  const queued = queueNewHeroAttacks(movedState, moved, ids.ABILITY_ZORO_ONI_GIRI, [target.id], {
...
function applyZoroAsura(state: GameState, unit: UnitState): ApplyResult {
  ...
  const targets = Object.values(state.units)
    .filter(
      (target) =>
        ...
        chebyshev(unit.position!, target.position) <= 2,
    )
    .map((target) => target.id);
  const committed = commitNamedResource(
    state,
  ...
  const queued = queueNewHeroAttacks(
    ...
    ids.ABILITY_ZORO_ASURA,
    targets,
    { damageBonus: 1, center: unit.position, radius: 2 },
  );
  return { state: queued.state, events: [...committed.events, ...queued.events] };
}
...
function applyDonReaction(state: GameState, unit: UnitState, action: AbilityAction): ApplyResult {
  ...
  if (
    ...
    return { state, events: [] };
  if (
    chebyshev(unit.position, destination) !== 1 ||
...
function applyDonWindmills(state: GameState, unit: UnitState, action: AbilityAction): ApplyResult {
  ...
  const crossed = path
    .slice(1, -1)
    .map((cell) => getUnitAt(state, cell))
    .filter((candidate): candidate is UnitState => !!candidate && candidate.owner !== unit.owner);
  ...
  events.push({
    ...
    unitId: moved.id,
    deltas: { [spec.id]: -3 },
    now: { [spec.id]: getCharges(moved, spec.id) },
  });
  ...
  if (repositionIds.length > 0) {
    ...
    const options = firstAffected?.position
      ? ALL_DIRS.map((dir) => ({
          col: firstAffected.position!.col + dir.col,
          row: firstAffected.position!.row + dir.row,
        }))
  ...
  const queued = queueNewHeroAttacks(movedState, moved, spec.id, [target.id], {
    center: target.position!,
  });
  return { state: queued.state, events: [...events, ...queued.events] };
}

function applyJackTrap(
  state: GameState,
  unit: UnitState,
  action: AbilityAction,
  isImpulse: boolean,
  ...
  if (
    !isImpulse ||
    unit.heroId !== HERO_JACK_RIPPER_ID ||
    !position ||
    !isInsideBoard(position, state.boardSize)
  )
    return { state, events: [] };
  return applyJackTrapPlacement(state, unit, position, explodePosition, rng);
}

function applyJackHolyMother(
  state: GameState,
  unit: UnitState,
  action: AbilityAction,
): ApplyResult {
  const targetId =
    typeof payload(action).targetId === "string" ? (payload(action).targetId as string) : "";
  const target = state.units[targetId];
  if (
    unit.heroId !== HERO_JACK_RIPPER_ID ||
    unit.jackHolyMotherUsed ||
    !target ||
    !target.isAlive ||
    !target.position ||
    target.owner === unit.owner
  ...
  const onTrap = (state.jackTraps ?? []).some(
    (trap) => trap.sourceUnitId === unit.id && coordsEqual(trap.position, target.position!),
  );
  const spec = getAbilitySpec(ids.ABILITY_JACK_RIPPER_DISMEMBERMENT);
  if (!onTrap || !spec || !canSpendSlots(unit, spec.actionCost?.consumes))
    return { state, events: [] };
  ...
  const queue = queueNewHeroAttacks(
    committed,
    updated,
    spec.id,
    [target.id, target.id, target.id, target.id],
    { damageOverride: 3, ignoreBonuses: true, center: target.position, preserveDuplicates: true },
  );
  return { state: queue.state, events: [abilityUsed(unit.id, spec.id), ...queue.events] };
}

function applyArtemisInsight(
  state: GameState,
  unit: UnitState,
  action: AbilityAction,
  rng: { next: () => number },
  ...
  if (
    ...
    return { state, events: [] };
  if (unit.blindUntilOwnTurnStart && chebyshev(unit.position, center) > 1)
    return { state, events: [] };
  ...
  const revealed = revealStealthedInArea(
    base,
    center,
    1,
    rng,
    (target) => target.owner !== unit.owner,
  );
  return {
    state: revealed.state,
    events: [
      abilityUsed(unit.id, ids.ABILITY_ARTEMIDA_MOONLIGHT_SHINE),
      ...revealed.events,
      {
        type: "aoeResolved",
        sourceUnitId: unit.id,
        abilityId: ids.ABILITY_ARTEMIDA_MOONLIGHT_SHINE,
        casterId: unit.id,
        center,
        radius: 1,
        affectedUnitIds: [],
        revealedUnitIds: revealed.events
...
function applyArtemisSickle(state: GameState, unit: UnitState, action: AbilityAction): ApplyResult {
  const toward = parseCoord(
    payload(action).target ?? payload(action).direction ?? payload(action).center,
  );
  if (
    unit.heroId !== HERO_ARTEMIDA_ID ||
    !unit.position ||
    ...
    return { state, events: [] };
  if (unit.blindUntilOwnTurnStart && chebyshev(unit.position, toward) > 1)
    return { state, events: [] };
  ...
  const queued = queueNewHeroAttacks(base, updated, spec.id, targets, {
    damageOverride: 2,
    ignoreBonuses: true,
...
export function applyNewBatchAbility(
  ...
  switch (action.abilityId) {
    case ids.ABILITY_DUOLINGO_PUSH_NOTIFICATION:
      return applyDuolingoPush(state, unit, action);
    case ids.ABILITY_LUCHE_DIVINE_RAY:
      return applyLucheLightRay(state, unit, action, isStartTurnImpulse);
    case ids.ABILITY_LUCHE_BURNING_SUN:
      return applyLucheFallingSun(state, unit);
    case ids.ABILITY_KANEKI_REGENERATION:
      return applyKanekiRegeneration(state, unit, action);
    case ids.ABILITY_ZORO_ONI_GIRI:
      return applyZoroOniGiri(state, unit, action, isStartTurnImpulse);
    case ids.ABILITY_ZORO_ASURA:
      return applyZoroAsura(state, unit);
    case ids.ABILITY_DON_KIHOTE_SORROWFUL_COUNTENANCE:
      return applyDonReaction(state, unit, action);
    case ids.ABILITY_DON_KIHOTE_WINDMILLS:
      return applyDonWindmills(state, unit, action);
    case ids.ABILITY_JACK_RIPPER_SNARES:
      return applyJackTrap(state, unit, action, isStartTurnImpulse, rng);
    case ids.ABILITY_JACK_RIPPER_DISMEMBERMENT:
      return applyJackHolyMother(state, unit, action);
    case ids.ABILITY_ARTEMIDA_MOONLIGHT_SHINE:
      return applyArtemisInsight(state, unit, action, rng, isStartTurnImpulse);
    case ids.ABILITY_ARTEMIDA_SILVER_CRESCENT:
      return applyArtemisSickle(state, unit, action);
    default:
      return null;
  }
}
